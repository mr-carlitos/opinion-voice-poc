...
def load_corpus(graph: Graph, folder: Folder) -> Corpus:
    ...
    for entry in entries:
        ...
        if "file" not in item or item.get("parentReference", {}).get("id") != folder.item_id:
        ...
        if latest.get("eTag") != item.get("eTag"):
            raise ServiceError("Quelle wurde waehrend des Lesens geaendert. Sitzung erneut starten.")
        reader = PdfReader(BytesIO(content))
        if reader.is_encrypted or not 1 <= len(reader.pages) <= 8:
            raise ServiceError("Nur unverschluesselte Text-PDFs mit maximal acht Seiten sind erlaubt.")
        ...
        versions[document_id] = item.get("eTag", "")
        for page_number, page in enumerate(reader.pages, start=1):
            ...
            total += len(text)
            if total > 100_000:
                raise ServiceError("Korpus zu gross: maximal 100000 Zeichen; keine stille Kuerzung.")
